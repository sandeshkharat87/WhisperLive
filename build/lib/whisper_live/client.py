...
class Client:
    ...
    def process_segments(self, segments):
        ...
        if self.log_transcription:
            ...
            self.global_text_collection.append(text)
            
            if self.handle_callback:
                # print(f"@@@ {type(text)} ",self.handle_callback(text))
                # print(f"@@@ {type(text)} ",self.handle_callback(text))
                print("--client @@@@",type(text))
                print(self.handle_callback(text))
...
class TranscriptionClient(TranscriptionTeeClient):
    ...
    def __init__(
        ...
        self.client = Client(
            ...
            max_connection_time=max_connection_time,handle_callback = handle_callback
        )
